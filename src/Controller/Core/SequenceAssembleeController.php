...
class SequenceAssembleeController extends AbstractController
{
	...
	public function indexjsonAction(Request $request, GenericFunctionE3s $service)
	{
		...
		$tab_toshow = [];
		$entities_toshow = $em
			->getRepository("App:SequenceAssemblee")
		...
		foreach ($entities_toshow as $entity) {
			...
			$query = $em->createQuery(
				'SELECT eaet.id, voc.libelle as gene, individu.codeIndBiomol as code_ind_biomol
			...
			$query = $em->createQuery(
				...
				JOIN a.sequenceAssembleeFk sqc  
				WHERE a.sequenceAssembleeFk = ' . $id
			)->getResult();
			$motuAssigne = (count($query) > 0) ? 1 : 0;
			// load the first identified taxon            
			$query = $em->createQuery(
				'SELECT ei.id, ei.dateIdentification, rt.taxname as taxname, voc.code as codeIdentification 
				FROM App:EspeceIdentifiee ei 
				JOIN ei.referentielTaxonFk rt 
				JOIN ei.critereIdentificationVocFk voc 
				WHERE ei.sequenceAssembleeFk = ' . $id . ' ORDER BY ei.id DESC'
			)->getResult();
			...
			$query = $em->createQuery(
				'SELECT s.codeSource as source 
			FROM App:SqcEstPublieDans sepd 
			JOIN sepd.sourceFk s 
			WHERE sepd.sequenceAssembleeFk = ' . $id
			)->getResult();
			...
			$tab_toshow[] = array(
				...
				"sequenceAssemblee.dateCre" => $DateCre,
				"sequenceAssemblee.dateMaj" => $DateMaj,
				"userCreId" => $service->GetUserCreId($entity),
				"sequenceAssemblee.userCre" => $service->GetUserCreUsername($entity),
				"sequenceAssemblee.userMaj" => $service->GetUserMajUsername($entity),
			);
		}
		// Ajax answer
	...
	public function indexjsonAction_backup(Request $request, GenericFunctionE3s $service)
	{
		...
		$entities_toshow = $em
			...
			->leftJoin(
				...
				'vocStatutSqcAss',
				'WITH',
				'sequenceAssemblee.statutSqcAssVocFk = vocStatutSqcAss.id'
			)
			->leftJoin(
				'App:EstAligneEtTraite',
				'eaet',
				'WITH',
				'eaet.sequenceAssembleeFk = sequenceAssemblee.id'
			)
			->leftJoin(
				'App:Chromatogramme',
				'chromatogramme',
				'WITH',
				'eaet.chromatogrammeFk = chromatogramme.id'
			)
			->leftJoin('App:Pcr', 'pcr', 'WITH', 'chromatogramme.pcrFk = pcr.id')
			->leftJoin('App:Voc', 'vocGene', 'WITH', 'pcr.geneVocFk = vocGene.id')
			->leftJoin('App:Adn', 'adn', 'WITH', 'pcr.adnFk = adn.id')
		...
		foreach ($entities_toshow as $entity) {
			...
			$query = $em->createQuery(
                ...
                JOIN ei.referentielTaxonFk rt 
                JOIN ei.critereIdentificationVocFk voc 
                WHERE ei.sequenceAssembleeFk = ' . $id .
					' ORDER BY ei.id DESC'
			)->getResult();
			$lastTaxname = ($query[0]['taxname'] !== NULL) ? $query[0]['taxname'] : NULL;
			$lastdateIdentification = ($query[0]['dateIdentification']  !== NULL)
