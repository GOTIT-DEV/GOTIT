<template>
  ...
  >
    ...
    <l-circle :lat-lng="[latitude, longitude]" :radius="10000" :opacity="0.5" />
    <l-layer-group layerType="overlay" :name="$t('nearby_sites_legend')">
      <shape-marker
        v-for="site in proximalSites"
        :key="site.id"
...
export default {
  ...
  methods: {
    ...
    continuousFit(epsilon) {
      const innerMap = this.$refs.map;
      setTimeout(async () => {
        innerMap.fitBounds(innerMap.data);
        await this.$nextTick();
        if (
    ...
    async fetch() {
      ...
      const response = await fetch(Routing.generate("proximal_stations"), {
        method: "POST",
        body: postData,
      });
      const fetchData = await response.json();
      this.proximalSites = fetchData.sites.map((site) => {
        return {
          ...site,
